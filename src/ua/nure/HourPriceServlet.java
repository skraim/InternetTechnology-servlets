...
@WebServlet(urlPatterns = "/inahourprice")
public class HourPriceServlet extends HttpServlet {
    ...
    public void doGet(HttpServletRequest req, HttpServletResponse res)
        ...
        Integer integer;
        try {
            integer = Integer.parseInt(req.getParameter("priceInAHour"));
        } catch (Exception ex) {
        ...
        for (int i = 0; i < list.size(); i++) {
            if (list.get(i).getId() == integer) {
                ...
                stringBuilder.append("Price for the rent in a hour = ")
                        .append(list.get(i).getPriceInHour().getValue())
                        .append(list.get(i).getPriceInHour().getCurrency());
